...
初版では、展開対象にLFSポインタ、サブモジュール、シンボリックリンク、Windowsで展開できない名前があると停止する。必要なプロファイル・分割モデル等は同一リポジトリに含める。SDKが未ロードとして返すモデルや出力警告がある場合も停止する。SDKが検出・通知しない依存欠落を完全に保証するものではないため、実機で図と本文の網羅性を照合する。
