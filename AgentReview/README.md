...
上位文書を指定済みなら、「レビュー開始」で自動的に取り込む。設定がない場合や指定が空の場合は、先にユーザーへ確認する。「設定する」を選ぶと入力設定を開いて開始を中断し、保存後に再度「レビュー開始」を押す。設定しない場合も、今回は上位文書なしで続行するかを確認し、明示的に続行を選んだ場合だけ開始する。続行の選択と整合が未確認であることは `inputs.md` に残す。指定したモデルID・ファイルが無効な場合は、黙って除外せず開始を止める。

### 上位文書の指定

| 設定状態 | 扱い |
|---|---|
| 未設定 | 設定するか毎回確認する。今回のみ上位文書なしで続行しても、設定状態は未設定のまま |
| 指定あり | 指定されたモデル・資料を取り込む |
| 意図的になし | 保存した理由を示し、今回も上位文書なしでよいか毎回確認する。断った場合は設定を開き、開始しない |

意図的に指定しない場合は、入力設定へ次を記載する。既存の設定ファイルに項目がなければ追記する。理由は必須で、モデル・資料の指定との併用はできない。

```ini
upstream.intentionalNone=true
upstream.noneReason=このプロジェクトが最上位の要求を定義するため
```

この設定はプロジェクト単位で保持されるが、次回の確認を省略する指定ではない。毎回の続行回答と確認日時を、保存された理由と区別して `inputs.md` に記録し、`review/coverage.md` にも引き継ぐ。通常の指定に戻す場合は `upstream.intentionalNone=false` に戻し、理由を空にする。「意図的になし」も上位整合の確認済みを意味しない。

1. 保存済みのプロジェクトを開き、「レビュー入力設定」を押す。
2. 設定ファイルの `upstream.model.1` にモデルID、`upstream.file.1` に外部資料のパスを記載する。どちらか一方のみでもよい。複数指定は末尾を `.2`、`.3` と増やす。
3. モデルIDは設定ファイルと同じフォルダの `model-catalog.tsv` で確認する。設定フォルダは出力ウィンドウに表示される。ファイルパスは絶対パス、またはプロジェクトフォルダからの相対パス。引用符は付けない。
4. 対象設計を選択して「レビュー開始」を押し、表示された上位モデル・資料を確認する。
5. エージェントとの対話で対象工程を指定する。指摘は `review/review.md`、要求対応表は `review/coverage.md` に出力され、「結果を開く」で確認できる。

設定は `%USERPROFILE%\.nd-agent-review\projects\<プロジェクトパスのハッシュ>\review-inputs.ini` に保持する。プロジェクトを移動すると別設定になる。元プロジェクトやプロファイルには書き込まない。
...
リンク／ジャンクション経由の資料は実体パスを指定する。コピーに失敗した場合はAIを起動せず、生成途中のセッションを再開対象から除外する。Excel・Word・PDFの内容読解はエージェント側の環境に依存し、読めなかった範囲は判断不能として残す。
