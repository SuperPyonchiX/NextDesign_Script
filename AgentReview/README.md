...
上位モデルと外部資料を指定する「上位整合レビュー」を追加。指定した入力とAttachmentを固定コピーし、要求の反映を `review/coverage.md` に記録する。操作と実機確認は [VERIFY.md](VERIFY.md) を参照。

| 機能 | 状態 |
|---|---|
| 単体レビュー | 維持。Attachmentはリンクから固定コピーへ変更 |
| 上位整合レビュー | 実装済み、模擬SDKで検証済み。Next Design実機は未確認 |
...
4. 対象設計を選択して「上位整合レビュー」を押し、表示された上位モデル・資料を確認する。
