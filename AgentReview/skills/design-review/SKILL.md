---
name: design-review
description: AgentReviewの設計出力を工程別にレビューし、指摘と修正提案を記録する。上位文書が指定された場合は要求・制約の反映を対応表で確認する。「レビューして」「上位要求の反映を確認して」「設計の漏れを見て」で使う。
metadata:
...
R1 入力一覧・対象設計・指定上位文書の読み込み
 ...
 → R4 レビュー実施（上位整合モードでは対応表も作成）→ 指摘一覧を書き、要約を提示
...
0. `session.ini` と `inputs.md` があれば読む。`state` が `preparing` / `failed` なら入力生成が未完了なのでレビューせず、再出力を依頼する。旧セッションで両ファイルや `mode` がなければ単体レビューとして扱う。`mode=upstream` の場合だけ [上位整合レビュー](references/upstream-review.md) を読む。現在、変化点レビューの入力生成は未提供。`mode=change` 等の未対応モードなら単体レビューに置き換えず、その旨を伝えて止める。
...
4. 上位整合モードでは `review/coverage.md` に指定上位文書の要求・制約を全件列挙し、指摘Noを対応付ける。判断不能や対象外を適合件数に含めない。全要求を列挙できない資料がある場合は、未読・未確認の範囲も明記し、網羅確認を完了としない。
...
- 上位整合モードでは指摘の修正・取り下げを対応表にも反映し、判断不能が解消した根拠を残す
