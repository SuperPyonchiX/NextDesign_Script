...
public static class InputTests
{
    ...
    public static void Run(string temp)
    {
        ...
        Throws(() => new ReviewInputs().ValidateUpstream(), "Reject empty upstream");
        ...
        command.StartUpstreamReview(context, new ICommandParams());
        Check(TerminalLauncher.Launches == 1, "Launch after upstream inputs");
        var session = SessionLocator.FindLatest(workspace);
        Check(session != null && session.Mode == "upstream" && session.State == "ready", "Ready upstream session");
        ...
        Check(session.Mode == "single" && !Directory.Exists(Path.Combine(session.Folder, "upstream")), "Single excludes configured upstream");
