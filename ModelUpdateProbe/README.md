...
ModelUpdateProbe 0.1.1 は、選択モデルの文字列属性1件を更新し、変更後と手動Undo/Redo後の値を記録する検証用拡張です。会社PCで実行し、結果の要約画面を撮影して開発側へ渡します。
...
- 初版は単値の `String` フィールドだけを対象とする。リッチテキスト・多値・数値・参照・所有フィールドは対象外。
...
プロジェクトを閉じる・切り替える・再読み込みする・Next Designを再起動する場合も再準備する。対象プロジェクトのオブジェクトが変わった場合や対象が削除済み・プロキシの場合は停止する。同じオブジェクトのままの再読み込みは自動検出できないため、再読み込み後の再準備は操作手順として必須。
...
| C003 | 準備時と現在のプロジェクトのオブジェクト参照が不一致。同じプロジェクトを開いたままか、切替・再読み込みの有無を添えて画面を渡す。SDKが別オブジェクトを返した可能性も未除外 |
| C004 / C005 | 準備時のモデルが削除済み / プロキシ。対象の状態を確認する |
