...
Next Design V3系で、PlantUMLから編集可能なシーケンス図を作成・更新するための調査記録。対象読者は拡張機能の開発担当者。2026-09-19時点で、最小図を生成する実装には未着手。公開SDKにインポートAPIを確認したが、入力形式の仕様が不足している。
...
現在の不足はCの入力仕様。モデルだけを追加しても、表示用シェイプや接続・順序の整合性を保証できない。差分更新には、さらに既存IDと外部参照の扱いを確定する必要がある。
...
したがって、`AddNewModel` と構造フィールドへの `SetField` を組み合わせた旧ライターを再公開しない。JSONのレコード名や形も推測で実装しない。
