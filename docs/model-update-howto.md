...
公式仕様では、`IField.Type` の文字列型名は `String`。上限多重度が1、所有でも参照でもなく、クラス型・列挙型でもないフィールドを対象とする。モデルの編集可否を確認してから `GetField` で変更前を取得し、確認画面の後にもう一度読み直す。
...
多値フィールドへの `SetField` は先頭要素を設定する。所有フィールドへの代入は既存モデルの削除や指定モデルの移動を伴うため、初版の文字列検証と混ぜない。リッチテキストも専用の検証対象とする。
...
| 実機で確認 | 会社PCの結果画面、ログ、観測報告。現時点では未受領 |
