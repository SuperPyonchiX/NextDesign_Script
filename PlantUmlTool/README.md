...
クラス図ではなく、クラス図グループ（例:「クラス設計」）を開いた・選んだ状態でも「PlantUMLから新規作成」を使えるようにした。新しい図はそのモデルの下（クラス図を持てるフィールド）に作る。雛形は、そのモデル配下でクラスが載っている最初のクラス図、無ければプロジェクト内の最初のクラス図を使い、確認ダイアログに名前を出す。クラス図を開いているときは従来どおりその図の隣に作る。実機未確認。
