...
## 2.3.4: 開いていない雛形のクラス図を読む

2.3.3 は「クラス設計」直下の「クラス図」をクラス図と判定したが、ノードが 0 件で C310 になった。このセッションで一度も開いていない図はノードが読み込まれていないとみている（推定）。ノードの無いクラス図しか見つからないときは、その図をナビゲータで選んで開き、読み直してから雛形にする。実行後は雛形の図が表示された状態になる。実機未確認。

## 2.3.3: 雛形の探索を直下だけにする

2.3.2 はボタンを押してからファイル選択が出るまでに長く待たされた。雛形のクラス図をモデルツリー全体（見つからなければプロジェクト全体）から探し、モデルごとにエディタを読んでいたため。探索は選んだモデルの直下だけにした。直下にクラスの載ったクラス図が無いときは、雛形にするクラス図を開いてから実行するよう案内して止まる。

## 2.3.2: 雛形のクラス図の探索を修正

2.3.1 では「クラス設計」の配下にクラス図があっても「雛形にできるクラス図がありません」（C310）で止まった。エディタを取得できないモデルがあると、その子を探さずに打ち切っていたのが原因とみている（推定）。エディタの取得と子の探索を分け、見つからないときは調べたモデル数・エディタ数・クラス図と判定したエディタ・最初の例外をメッセージに出す。実機未確認。

## 2.3.1: クラス図グループから新規作成

クラス図ではなく、クラス図グループ（例:「クラス設計」）を開いた・選んだ状態でも「PlantUMLから新規作成」を使えるようにした。新しい図はそのモデルの下（クラス図を持てるフィールド）に作る。雛形は、そのモデルの直下でクラスが載っている最初のクラス図を使い（2.3.3 から。それ以前はツリー全体を探した）、確認ダイアログに名前を出す。クラス図を開いているときは従来どおりその図の隣に作る。実機未確認。
