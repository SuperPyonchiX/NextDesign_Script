...
## 2.3.0: PlantUML から新しいクラス図を作る

「反映（クラス図）」グループに「PlantUMLから新規作成」を追加した。開いているクラス図を雛形にして、PlantUML ファイルの内容で新しいクラス図を作る。実機未確認。

1. 雛形にするクラス図を開き、プロジェクトを保存しておく（未保存の変更があると止まる）。
2. 「PlantUMLから新規作成」→ `.puml` を選ぶ → 作成の確認 → 反映内容の確認。
3. 新しい図は雛形の図のモデルと同じ所有先に、同じメタクラス・ビュー定義で作られる。図の名前は `title` 行、無ければファイル名。

クラスの扱い:

| PlantUML のクラス | 新しい図での扱い |
|---|---|
| 雛形の図に同じ名前のクラスがある | 既存のクラスをそのまま載せる。本体 `{ }` を書かなければメンバはそのまま、書けばその内容に合わせる（「PlantUMLを反映」と同じ） |
| 雛形の図に無い | 新しく作る。所有先とメタクラスは、雛形の図にある同じ種類（キーワード・ステレオタイプ）のクラスから取る |

- ステレオタイプを省略したクラスは、同じキーワードの雛形クラスのステレオタイプを引き継ぐ。書いた場合は、雛形の図に同じステレオタイプのクラスが要る。
- `package` の囲みは無視する（所有先は雛形で決まる）。`package` / `component` の箱をクラスとして書いたもの、入れ子のクラスは扱えない。
- 既存のクラス同士の関連は、PlantUML に書いていなくても消さない。
- 雛形の図に無いが同じ所有先に同じ名前のクラスがあると止まる（既存クラスを使いたいときは、先に雛形の図へ載せる）。
- 途中で 1 回プロジェクトを保存する（関連の線を表示するための図の退避に必要）。反映が失敗・中止したときは、作成した図と仮に作ったクラスを削除する（保存済みのファイルには残るので、上書き保存で確定する）。
- 関連の線の形は雛形の図の最初の線を複製する。雛形の図に線が 1 本も無いと関連を追加できない。

同時に、反映本体（`63-class-sync-runtime.cs`）へ次を足した。通常の「PlantUMLを反映」の動きは変わらない。作成中だけ有効になる。

- 線の無い図でも、作成側が渡した雛形の線で関連を表示する。
- メンバの無いクラスへメンバを足すとき、作成側が渡した雛形クラスのメンバのメタクラス（Method など）を使う。
- 同じクラスの隣に複数のクラスを足すとき、重ならないよう下へずらす。
...
| クラス図 | 対応 | 対応（2.2.0 で既存の図への差分反映、2.3.0 で新しい図の作成） |
| 状態遷移図 | 対応 | 未対応 |
...
| 反映（クラス図） | PlantUMLから新規作成 | 表示中のクラス図を雛形に、`.puml` の内容で新しいクラス図を作る（2.3.0） |
