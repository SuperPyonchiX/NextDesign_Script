...
## 3.3.4: 未保存のときは保存してから更新するか聞く

シーケンス図の「PlantUMLで更新」は、未保存のプロジェクトで押すと「保存してから更新する（Ctrl+Z で戻せる）／保存せずに更新する」を聞く。保存せずに更新したあとの Ctrl+Z は、図を最後に保存した状態の図形に戻す（製品のエディタ取込の Undo の挙動。保存後に追加した図形は、更新で触れていなくても消える）。MCP など呼び出し元が UpdateWithoutSaving を立てた場合は聞かない。実機未確認。
