...
実機（ClassImportProbe 0.8.2 の箱置き調査）: 作ったばかりのクラス図は、確定後でも AddNodeShape を「ビュー定義 … を用いて … シェイプを追加できません」で拒否し、エディタに開いた（SetCurrentModel）後は Domain・Unit とも置けた。箱の定義はどれもクラスへのマッピングで、図のモデルからの参照は要らない（2.4.4 の推定は誤り。このプロファイルの図のモデルには Domain を参照する欄が無く C320 で止まっていた）。図のモデルを作って確定し、開いてから箱を置く。途中で止まったら作った図を削除する。実機未確認。
