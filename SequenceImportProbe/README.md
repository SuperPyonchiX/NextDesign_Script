...
削除対象の子要素が外部モデルと関連している場合は、変更前に停止する。図本体の関連は更新後にも照合する。削除と再取り込みは同じトランザクションに入れ、不一致や例外では取消を試み、旧モデルの名前・関連・図形IDの復元を診断に記録する。自動保存はしない。`before.txt` は診断用の構造記録で、単独で復元できるバックアップではない。

実機で確認済みなのは0.4.0の一時図に対する同一IDの名称更新と、一時モデルを含む取消まで。0.5.1の子要素削除・追加、既存エディタへの反映、外部参照保持、保存再読込、Undo/Redoは実機未確認。
