...
0.3.6では `destroy A` によるライフラインの破棄点を追加する。既存のメッセージ種別を維持し、独立した破棄点として生成する。実行区間は破棄位置で閉じ、破棄後の送受信・再activate・二重破棄は事前検査で停止する。`loop` の外で始めた実行区間を内側で閉じる配置も扱う。検証用は `samples/09-destroy.puml`。生成データ・公式SDKコンパイルは検査済み、破棄点の実機生成・表示は未確認。
